...
fn main() {
    let ns = vec![10, 100, 1000, 10000, 50000, 100000, 500000, 1000000];
    ...
    for n in ns {
        ...
        let mut v1: Vec<i32> = (0..n).map(|_| rng.gen_range(0..=5000)).collect();
        let v2 = v1.clone();
        ...
        quick_sort_time.push((n, duration.as_secs_f64()));

        // 2. select
        let start = Instant::now();
        let select_kth = select_kth(v2, k);
        let duration = start.elapsed();
        select_time.push((n, duration.as_secs_f64()));

        assert_eq!(quick_sort_kth, select_kth);
    }

    for q in quick_sort_time {
        println!("{:?}", q);
    }

    for s in select_time {
        println!("{:?}", s);
    }
}
